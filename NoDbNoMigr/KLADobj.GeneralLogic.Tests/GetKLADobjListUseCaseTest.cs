    ...
    public class GetKLADobjListUseCaseTest
    {
        private InMemoryKLADobjRepository CreateKLADobjRepository()
        {
            var repo = new InMemoryKLADobjRepository(new List<kladobj> {
                new kladobj { Id = 1, CloseFlag = "закрытое", Name = "Домодедовское кладбище"},
                new kladobj { Id = 2, CloseFlag = "закрытое", Name = "Жохово кладбище"},
                new kladobj { Id = 3, CloseFlag = "открытое", Name = "Захарьинское кладбище"},
                new kladobj { Id = 4, CloseFlag = "закрытое", Name = "Зеленоградское (Северное) кладбище"},
            });
            return repo;
        }
        [Fact]
